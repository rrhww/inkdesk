...
Inkdesk 当前的产品主形态已经从“私有 LLM Wiki”继续收敛为：
...
对外部 Agent 来说，Inkdesk 不是另一个聊天应用，而是可接入的通用知识底座。Claude Code、Codex、Cursor 等工具可以通过 MCP / CLI / 项目级 Skill 调用 Inkdesk：任务前获取相关长期上下文，任务后把高价值回答沉淀回知识库。
...
- Inkdesk 通过 MCP / CLI / Skill 形态成为外部 Agent 的长期知识底座。

## 产品目标

Inkdesk 的目标不是成为更复杂的知识库编辑器，而是成为一层长期可用的 Agent Knowledge Runtime：
...
- [x] 私有 owner 登录与 `/app` 工作区
- [x] `raw -> ingest -> wiki -> ask` 主闭环
- [x] raw 文本、网页、PDF 导入与 vault markdown 持久化
- [x] AI 编译提案、人工接受 / 驳回、wiki 写入
- [x] Ask-first 工作区、引用回答、追问链与 writeback 提案
- [x] claim 级证据状态、使用次数、最近验证时间、冲突/过期信号
- [x] 本地 Docker 全栈：Next.js、FastAPI、PostgreSQL + pgvector
- [x] 后端 pytest 与前端 TypeScript typecheck 通过
- [ ] 把“沉淀这次回答”做成每个回答的主操作
- [ ] 支持沉淀完整回答与沉淀选中片段
- [ ] 将沉淀后台流程拆成专项子 Agent：提取、证据绑定、路由、冲突检测、patch、gate
- [ ] 暴露 Inkdesk MCP / CLI 接口，让 Claude Code、Codex 等外部 Agent 接入同一知识底座
...
- `inkdesk-local-postgres`
- `inkdesk-local-server`
- `inkdesk-local-web`
...
```env
INKDESK_AUTH_SECRET=换成一个足够长的随机字符串
