...
- 渲染 Dev Run Console、Context Ask、raw / ingest / wiki / compile / health 页面
- 展示提案审阅、wiki 页面、问答结果和沉淀入口

### 后端职责

- 维护 raw / ingest / wiki / ask / run / deposit / compile / health API
- 后续维护 Skill Run / Evaluation API
- 读写 vault markdown
- 编排 LangGraph Ask / Compile runtime
- 维护数据库索引、提案队列和问答记录
- 提供 deposit orchestration 与 MCP 接入

### 数据库职责

- 存储 workspace、source、topic、review、ask turn 等索引数据
...
1. 直接访问 `/app`
2. 前端渲染工作区
3. 前端通过同源 `/api/**` 代理向后端发起 raw、ingest、wiki、ask、run 请求
