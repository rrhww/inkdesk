...
5. 默认控制在 1-3 个连续开发会话；预计超出时在编码前继续拆分。
6. 行为变更和 Bug 修复由 Codex 先写失败测试并确认预期失败，随后实现最小生产变更。
7. `web/**` 可见变更必须由 Codex 在真实浏览器验收，并向用户报告流程、截图或检查结果。
8. Codex 在计划完成后更新 `cognitive-map.md`，记录已确认边界、剩余模糊区和黑盒区。
...
前三个月只承诺完成 P0 的交付与验证，不以“功能数量”作为成功标准。
...
这是优先级节奏，不是固定工期承诺。任一小计划超过三个开发会话仍无法形成独立验收，应暂停并进一步拆分，而不是压缩测试、扩大范围或积累未验证变更。

## 21. 每个小计划的端到端执行循环

1. Codex 根据本路线图编写当前小计划的详细实施文档，核对旧代码、文件边界和验收标准。
2. 用户确认产品目标、不可逆选择和高影响取舍；普通实现细节由 Codex 按仓库模式决定。
3. Codex 先写失败测试并执行，确认失败原因与目标一致。
4. Codex 实现最小生产变更，并保持兼容、迁移和回滚边界。
5. Codex 执行相关单元、集成、契约、构建和安全验证。
6. `web/**` 可见变更由 Codex 在真实浏览器检查交互、响应式、控制台和关键资源。
7. Codex 自审正确性、领域设计、安全、测试质量和文档一致性，修复发现的问题后重新验证。
8. Codex 更新认知地图与计划状态，向用户报告变更、证据和剩余风险，再激活下一个计划。
