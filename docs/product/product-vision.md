...
Inkvault 的核心不是问答，也不是 wiki，而是一个面向 AI Agent 的个人知识底座。
...
Inkvault 是一个 Agent Knowledge Runtime：把 AI 回答和外部 Agent 工作过程里的高价值判断，沉淀成可追溯、可审阅、可修正、可复用的长期知识。
...
Inkvault 的长期产品目标，是成为外部 Agent 的知识底座。

Claude Code、Codex、Cursor 等 Agent 可以通过 MCP、CLI 或项目级 Skill 接入 Inkvault：

- 任务开始前：调用 Inkvault 获取相关长期知识和历史判断。
- 任务进行中：查询某个 topic、decision、open question 或 source。
- 任务结束后：把回答、方案、复盘或用户选中的片段提交给 Inkvault 沉淀。

外部 Agent 不需要理解 Inkvault 内部的 claim、review、health、vault patch。它们只需要两类能力：
...
这让 Inkvault 从“自己的知识 App”升级为“所有 Agent 都能调用的长期记忆层”。
...
### Inkvault as Agent Knowledge Runtime

Inkvault 通过 MCP / CLI / Skill 接入外部 Agent，为 Claude Code、Codex、Cursor 等工具提供长期知识上下文和沉淀接口。
