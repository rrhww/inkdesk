...
- 使用目标：在任务前获取 Inkvault 的长期知识上下文，在任务后把值得保留的结论交给 Inkvault 沉淀
...
1. Claude Code、Codex 或 Cursor 通过 MCP / CLI 请求 Inkvault context pack
2. 外部 Agent 使用相关长期知识完成任务
3. 用户要求沉淀时，外部 Agent 调用 Inkvault deposit
4. Inkvault 复用后台沉淀流水线，不要求外部 Agent 理解内部知识协议
