...
### `INKDESK_MIGRATION_LOCK_TIMEOUT_SECONDS`

- 用途：PostgreSQL migration advisory lock 的最长等待时间
- 示例：`30`
- 说明：到期返回 `DB_MIGRATION_LOCK_TIMEOUT`，应用不会绕过锁继续启动

### Durable Job variables

- `INKDESK_JOB_BACKEND`: `durable` (default) or emergency `legacy` fallback.
- `INKDESK_JOB_POLL_INTERVAL_SECONDS`: durable worker poll interval; default `1`.
- `INKDESK_JOB_LEASE_SECONDS`: claim lease duration; default `60`.
- `INKDESK_JOB_HEARTBEAT_SECONDS`: independent control-session heartbeat interval; default `10` and must remain below half the lease duration.
- `INKDESK_JOB_SHUTDOWN_GRACE_SECONDS`: graceful shutdown allowance; default `10`.
- `INKDESK_JOB_DEFAULT_MAX_ATTEMPTS`: automatic recovery budget; default `3`.
