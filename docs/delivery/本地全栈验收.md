...
这份文档用于验收 Inkdesk 当前已实现的 Ask / vault-first 本地闭环可用性。

产品目标已经调整为 Dev Run-first；在 Dev Run Console 落地前，本文件仍保留 Ask 闭环作为当前代码基线验收。Dev Run 落地后，需要新增 PRD / bug / 改造任务入口验收。

当前闭环定义为：

- 真实 owner 登录
- `/app` 当前作为 Ask 工作区可用；目标是改为 Dev Run Console
...
3. 在 `/app` 验证当前 Ask 首屏可见，并能在 5 秒内看懂 briefing summary、建议提问、知识缺口与下一步动作
...
9. 打开 `/app/ask`，确认它仍可访问同一 Ask 工作区；后续应作为 Context Ask 保留
...
- claim 治理信息能在 wiki / ingest / Ask 判断面板中被直接识别；unsupported / stale / conflicting claim 若当前数据尚未产生，不应作为首轮全链路失败条件

## 下一阶段新增验收

Dev Run Console 落地后，需要补充：

- `/app` 首屏展示 PRD / bug / 改造任务入口
- `/app` 展示阶段轨道：Context Pack、技术方案、评审、coding、测试、沉淀
- `/app` 展示待确认阶段输出和可用 Skills 摘要
- `/app/ask` 作为 Context Ask，而不是首页主工作区
