...
当前产品只有一个私有工作区，无需登录直接进入：

```mermaid
graph TD
    A["Inkdesk"] --> B["根路由 /"]
    B --> B1["直接重定向到 /app"]
    B1 --> D1["Dev Run Console /app"]
    B1 --> D2["Context Ask /app/ask"]
    B1 --> D3["Raw /app/raw"]
    B1 --> D4["Ingest /app/ingest"]
    B1 --> D5["Wiki /app/wiki"]
    D5 --> D6["Wiki Detail /app/wiki/[id]"]
    B1 --> D7["Runs /app/runs"]
    B1 --> D8["Health /app/health"]
    B1 --> D9["Compile /app/compile"]
```
...
```mermaid
flowchart LR
    A["/app"] --> B["Create Dev Run"]
    B --> C["Context Pack / Context Ask"]
    C --> D["Skill Output"]
    D --> E["Review / Gate"]
    E --> F["Deposit"]
    F --> G["Wiki / Runs / Evals"]
