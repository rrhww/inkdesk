...
**目标：** 在 Dev Run-first 产品主入口下，把 Context Ask 的回答沉淀能力做稳。`Ask -> 沉淀` 不再是 Inkdesk 的主产品路径，而是 `PRD / 研发任务 -> Dev Run -> Context Ask -> Deposit` 里的上下文沉淀子链路。

**当前定位：** 本计划是 Dev Run Console 的配套实施计划，不是产品根计划。真正的用户主路径应由 `/app` Dev Run Console 承载；本计划负责让 `/app/ask` 中的完整回答和选中片段能安全进入 review-first 沉淀流程。
...
下一个开发切片首先不是 Skill Workbench，也不是把 Ask 继续做成首页。

文章里的 `Claude Code + Skill repo + Obsidian` 形态很有参考价值，但 Inkdesk 应该先把两个入口边界做稳定：`/app` 是 Dev Run Console，`/app/ask` 是 Context Ask。如果 Context Ask 的沉淀不清晰，Skill Workbench、MCP 和外部 Agent 沉淀只会在模糊写入路径上增加更多入口。
...
```text
Dev Run -> Context Ask answer -> deposit request -> review proposal -> owner accepts -> wiki patch
```
...
- 在 `/app/ask` 中沉淀整段回答。
- 在 `/app/ask` 中沉淀选中片段。
- 从 Dev Run 阶段输出进入相同 deposit 契约。

## 交付顺序

0. 先确认 `/app` 的产品目标是 Dev Run Console，Ask 页面只保留在 `/app/ask`。
1. 在现有 writeback 之上稳定后端沉淀契约。
2. 增加整段回答沉淀反馈和幂等性。
3. 增加选中片段沉淀。
4. 把 Context Ask UI 升级成明确的沉淀入口。
5. 增加待沉淀内容的健康度和就绪信号。
6. 更新文档，然后进入 Skill Workbench / MCP / Dev Run event 对接。
...
- 修改： `web/tests/research-pages.test.tsx`
  - Assert the Context Ask page exposes the new deposit surface.
- 修改： `web/tests/research-api.test.tsx`
  - Assert frontend API wrappers call the correct endpoints.
- 修改： `docs/product/产品路线图.md`
  - Mark this plan as the active Context Ask deposit slice, not the product root slice.
- 修改： `docs/design/信息架构.md`
  - Confirm `/app` is Dev Run Console and `/app/ask` is Context Ask.
...
- 当 `canWriteback` 为 true 时，每条 Context Ask 回答都有固定的 `沉淀这次回答` 动作。
- 用户可以沉淀整段回答。
- 用户可以沉淀回答中的选中文本。
