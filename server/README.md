...
Inkdesk 的 Python 主后端，当前服务于单人私有、Vault-first 的研究工作台。
...
- `INKDESK_VAULT_ROOT` 可配置 vault 根目录，默认 `./inkdesk-vault`
...
2. 设置一个足够长的 `INKDESK_AUTH_SECRET`
...
```powershell
$env:INKDESK_DB_URL='postgresql+psycopg://inkdesk:inkdesk@localhost:5432/inkdesk'
...
$env:INKDESK_AGENT_RUNTIME='langgraph'
$env:INKDESK_AGENT_PROVIDER_PROFILE='openai'
$env:OPENAI_API_KEY='sk-...'
$env:INKDESK_AGENT_MODEL='gpt-4.1-mini'
$env:INKDESK_AGENT_CONNECT_TIMEOUT_SECONDS='2'
$env:INKDESK_AGENT_READ_TIMEOUT_SECONDS='20'
$env:INKDESK_ENABLE_WEB_ASSIST='true'
python -m uvicorn inkdesk_server.main:app --host 0.0.0.0 --port 8080
```
...
```powershell
$env:INKDESK_AGENT_PROVIDER_PROFILE='deepseek'
$env:DEEPSEEK_API_KEY='sk-...'
$env:INKDESK_AGENT_MODEL='deepseek-v4-flash'
python -m uvicorn inkdesk_server.main:app --host 0.0.0.0 --port 8080
```

如果未设置对应 profile 的 API key，或者显式把 `INKDESK_AGENT_RUNTIME` 设为 `deterministic`，Ask / Compile 会继续走 deterministic fallback，不会尝试调用真实 LLM。
...
- `openai_compatible` / `custom`：允许通过 `INKDESK_AGENT_BASE_URL` 与 `INKDESK_AGENT_API_KEY` 显式覆盖

如果 `INKDESK_ENABLE_WEB_ASSIST='true'`，`vault_plus_web` 模式会在 Ask 初答仍有知识缺口时显式联网补料；如果设为 `false`，Ask 会停留在纯 vault 回答，不会请求外部搜索结果。
