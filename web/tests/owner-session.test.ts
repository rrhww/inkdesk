...
test("owner session helpers define the hidden owner entry and protect the private system", async () => {
  ...
  assert.equal(module.OWNER_SESSION_COOKIE, "inkdesk_owner_session");
  ...
  assert.equal(module.resolveRootDestination(undefined), "public");
  ...
  assert.equal(module.resolveProtectedAppRedirect("/app/library", "session-token-value"), null);
});
