...
test("sidebar becomes a task-first research rail", async () => {
  const { AppSidebarContent } = await import("../components/app-sidebar");
  const html = compact(renderToStaticMarkup(AppSidebarContent({ pathname: "/app/ingest", snapshot: researchDashboardFixture, devRuns: [] })));
  const markers = [
    'href="/app"',
    'href="/app/ingest"',
  ];
  ...
  assert.match(html, /Dev Run/);
  assert.match(html, /Dev Run/);
  assert.match(html, /新建任务/);
  assert.match(html, /待办追踪/);
...
test("raw page presents imported files waiting for ingest", async () => {
  ...
  assert.match(html, /最近资料/);
  assert.match(html, /INGEST_PENDING/);
