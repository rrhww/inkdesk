...
describe("app shell primitives", () => {
  ...
  it("exposes Chinese primary sections in product order", () => {
    expect(PRIMARY_SECTIONS.map((item) => item.label)).toEqual(["任务", "问答", "资料", "审阅", "知识库", "技能", "健康", "编译"]);
    expect(PRIMARY_SECTIONS.map((item) => item.href)).toEqual(["/app", "/app/ask", "/app/raw", "/app/ingest", "/app/wiki", "/app/skills", "/app/health", "/app/compile"]);
