...
export default function AppError({
  ...
  return (
    <main className="mx-auto max-w-shell px-6 py-16 lg:px-8">
      <div className="text-[11px] uppercase tracking-[0.2em] text-ink-muted">页面错误</div>
      ...
      <p className="mt-5 max-w-2xl text-sm leading-7 text-ink-muted">
        {error.message || "可以先重试一次，或者回到知识拓扑继续查看。"}
      </p>
      <div className="mt-8 flex gap-3">
        <button className="rounded-sm bg-ink-primary px-5 py-3 text-sm font-semibold text-white" onClick={reset} type="button">
          重试
        </button>
        <Link href="/app/wiki" className="rounded-sm border border-ink-line bg-white px-5 py-3 text-sm font-semibold text-ink-text">
          回到知识拓扑
        </Link>
      </div>
    </main>
